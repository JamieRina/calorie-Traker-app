...
interface AppContextType {
  ...
  addCustomRecipe: (recipe: Omit<Food, "id" | "source">) => Food;
}

const UI_MODE_STORAGE_KEY = "nutritrack-ui-mode";
const THEME_MODE_STORAGE_KEY = "nutritrack-theme-mode";
const CUSTOM_FOODS_STORAGE_KEY = "bitebalance-custom-foods";
const CUSTOM_RECIPES_STORAGE_KEY = "bitebalance-custom-recipes";
...
function readStoredFoods(storageKey: string): Food[] {
  if (typeof window === "undefined") {
    return [];
  }
...
function makeLocalFoodId(prefix: string) {
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [customFoods, setCustomFoods] = useState<Food[]>(() => readStoredFoods(CUSTOM_FOODS_STORAGE_KEY));
  const [customRecipes, setCustomRecipes] = useState<Food[]>(() => readStoredFoods(CUSTOM_RECIPES_STORAGE_KEY));
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem(CUSTOM_RECIPES_STORAGE_KEY, JSON.stringify(customRecipes));
    }
  }, [customRecipes]);
  ...
  const addCustomFood = useCallback((food: Omit<Food, "id">) => {
    const savedFood: Food = { ...food, id: makeLocalFoodId("local-food") };
    setCustomFoods((previous) => mergeFoods(previous, [savedFood]));
    setRememberedFoods((previous) => mergeFoods(previous, [savedFood]));
    return savedFood;
  }, []);

  const addCustomRecipe = useCallback((recipe: Omit<Food, "id" | "source">) => {
    const savedRecipe: Food = { ...recipe, id: makeLocalFoodId("local-recipe"), source: "Recipe" };
    setCustomRecipes((previous) => mergeFoods(previous, [savedRecipe]));
    setRememberedFoods((previous) => mergeFoods(previous, [savedRecipe]));
    return savedRecipe;
  }, []);
